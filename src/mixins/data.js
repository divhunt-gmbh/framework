...
const Data =
{
    ...
    DataDefineOne(value, config, depth = 0)
    {
        ...
        try
        {
            ...
            if(Array.isArray(value))
            {
                ...
                if(parsed.required && !value.length)
                {
                    throw new Error('Required array is empty.');
                }
            }
    ...
    DataParseConfig(config)
    {
        ...
        {
            if(!['number', 'string', 'boolean', 'object', 'array', 'function'].includes(type))
            {
                throw new Error('Invalid config type. Expected number, string, boolean, object, array or function.');
            }
        })
